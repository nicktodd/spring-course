# Spring Course Labs

# January 2026 Exercises for the Spring with MySQL and Maven Pathway

[JPA with Hibernate](docs/jpa.md)

[Going Further with JPA](docs/further-jpa.md)

[Core Spring](docs/GettingStartedWithSpring.md)

[Working with SpringBoot and JPA](docs/jpa-boot.md)

[Adding the REST API](docs/springboot-jpa.md)

[Testing your Spring Boot Application](docs/AddingSomeTests.md)


<!--# Exercises for the Spring with MongoDB and Gradle Pathway

[Core Spring](docs/GettingStartedWithSpring.md)

[Working with Spring and MongoDB](docs/WorkingWithMongodbGradle.md)

[Working with Spring Data and MongoDB](docs/Springdatawithmongogradle.md)

[Spring Boot with Mongo, REST API and Gradle](docs/springbootmongorestgradle.md)
-->
