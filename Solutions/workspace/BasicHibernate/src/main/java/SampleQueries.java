...
public class SampleQueries {
	...
	public static void main(String[] args) {
		...
		System.out.println("Pagination end");
			...
			// use the named query
			/*Query query = em.createNamedQuery("compactdisc.findByPrice");
			query.setParameter("price", 12.0);
		...
		for(CompactDisc disc : discs) {
			...
			Query allCDsBeginningWithA = em.createQuery("select cd from CompactDisc as cd where cd.price > 12 order by cd.title");
...
//		Query allCDsWithTracks = em.createQuery("select cd from CompactDisc cd, Track t where count(cd.trackTitles) > 0");
